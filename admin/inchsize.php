...
switch ($_GET['action']){
	case 'del' :
		$id = $_GET['id'];
		$r = getRecord("tbl_inchsize","id=".$id);
		$resultParent = mysql_query("select id from tbl_inchsize where parent='".$id."'",$conn);
		if (mysql_num_rows($resultParent) <= 0){
			@$result = mysql_query("delete from tbl_inchsize where id='".$id."'",$conn);
			if ($result){
...
<input type="hidden" name="act" value="inchsize">
<?
$pageindex = createPage(countRecord("tbl_inchsize",$where),"./?act=inchsize&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
...
<table border="1" cellpadding="2" bordercolor="#C9C9C9" width="100%">
	<tr>
	  <th width="60" class="title"><input type="checkbox" name="chkall" onClick="chkallClick(this);"></th>
		...
		<th width="476" class="title"><a class="title" href="<?=getLinkSort(2)?>">Inchsize</a></th>
...
$sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd from tbl_inchsize where $where $sortby $direction limit ".($p*$MAXPAGE).",".$MAXPAGE;
...
while($row=mysql_fetch_array($result)){
	...
	<tr>
		...
		<td width="40" align="center" bgcolor="<?=$color?>" class="smallfont">
			<a href="./?act=inchsize_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>">Sửa</a>		</td>
		<td width="40" align="center" bgcolor="<?=$color?>" class="smallfont">
			<a 
				onClick="return confirm('Bạn có chắc chắn muốn xóa ?');" 
				href="./?act=inchsize&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>"
			>Xóa</a>	  </td>
		<td bgcolor="<?=$color?>" class="smallfont" align="center"><?=$row['id']?></td>
		<td bgcolor="<?=$color?>" class="smallfont"><?=$row['inchsize']?></td>
...
<input type="button" value="Thêm mới" name="btnNew" onClick="window.location='./?act=inchsize_m&page=<?=$_REQUEST['page']?>&cat=<?=$_REQUEST['cat']?>'" class="button">
</form>
...
<table width="100%">
	...
	<tr><td class="smallfont"><?='Tổng số sách : <b>'.countRecord('tbl_inchsize','id>4').'</b>'?></td></tr>
</table>