...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
	  <td align="center" style="padding-bottom:10px"><span class="text_pro"><?=$pro['name']?> </span></td>
  </tr>
	<tr>
	  <td align="center"><img src="<?=$pro['image_large']?>" border="0" /></td>
  </tr>
  <tr>
	  <td align="center" style="padding-bottom:10px; padding-top:10px">
	  	<?=_PRICE?> : <span class="red"><strong><?=number_format($pro['price'],0,',','.')?>&nbsp;<?=$currencyUnit?></strong></span></td>
  </tr>
  <? if($pro['subject']>0)
					{?>
				...
				?>			
  <tr>
	  <td align="center" style="padding-bottom:10px"><a href="./?frame=cart&p=<?=$pro['id']?>" class="link">
	  	<img src="images/cart.jpg" width="20" height="17" vspace="2" border="0" align="absmiddle" />&nbsp;&nbsp;Đặt hàng &nbsp;</a></td>
  </tr>
		<tr>
			<td align="justify" style="padding-bottom:10px; padding-top:10px; padding-left:10px"><?=$pro['detail_short']?></td>
		</tr>
</table>
<br />
<?
...
$sql = "select * from tbl_product where status=0 AND id<>'".$pro['id']."' AND parent=".$pro['parent']." order by sort,date_added desc limit ".$row*$col*$p.",".$row*$col;
...
<table width="98%" border="0" cellpadding="0" cellspacing="0">
  <tr>
   ...
   <tr>
    <td style="padding-top:10px"><span style="font-family:Tahoma; font-size:12px; color:#333333; padding-left:5px;padding-top:10px"><strong>Sản phẩm khác : </strong></span></td>
  </tr>
</table>
...
<IMG src="<?=$pro['image']; ?>" width="180" border=0 onmouseover="showtrail('<? if ($pro['image_large']!='') echo $pro['image_large']; else echo $pro['image']; ?>','',<?=$width; ?>,<?=$height;?>)" onmouseout=hidetrail()></A>
				...
				<tr>
				  <td height="25" colspan="2" align="center" valign="bottom"><?=_PRICE?>: <span class="red"><strong><?=number_format($pro['price'],0,',','.')?>&nbsp;<?=$currencyUnit?></strong></span> </td>
				</tr>
				<? if($pro['subject']!=0)
					{?>
					<tr><td align="center" style="padding:2px" colspan="2" height="25" valign="bottom">
						 <?=$_lang=='vn' ? 'Giá khuyến mãi :' : 'Promotion Price : '?> 
						 <span class="red"><strong><?=number_format($pro['subject'],0,',','.')?>&nbsp;<?=$currencyUnit?></strong></span>
					</td></tr>					
					<? }
...
echo '<tr><td class="smallfont" align="left" style="padding-top:5px"><b>'.$total.'</b> '.$newsPage.'</td>';
