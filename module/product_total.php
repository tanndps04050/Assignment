...
$sql="select * from tbl_product where parent = ".$cat1." order by sort desc limit ".$row*$col*$p.",".$row*$col;
$result = @mysql_query($sql,$conn);

$total = countRecord("tbl_product","status=0 and parent=".$cat1);
if($total==0){
?>
...
<table align="center" cellSpacing="0" cellPadding="0" width="100%" border="0">
	...
	<tr>
		<td align="center">
			<font color="#000000"><b><?=$_lang=="vn"?'Sách đang cập nhật !':'Products are being updated !'?></b></font>
		</td>
	</tr>
	<tr><td height="20"></td></tr>
</table>
...
for($j=0; $j<$col&&$pro=mysql_fetch_assoc($result); $j++){				
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="float:left">
				...
				<tr>
				  <td colspan="2" align="center" height="180" valign="middle"><a href="./?frame=product_detail&id=<?=$pro['id'];?>"><img src="<?=$pro['image']; ?>" width="180" border=0 onmouseover="showtrail('<? if ($pro['image_large']!='') echo $pro['image_large']; else echo $pro['image']; ?>','',<?=$width; ?>,<?=$height;?>)" onmouseout=hidetrail() /></a>
			      <DIV id=preview_div style="DISPLAY: none;  POSITION: absolute"></DIV>	 </td>
				</tr>
				<tr>
				  <td height="25" colspan="2" align="center"><?=_PRICE?>: <span class="red"><strong><?=number_format($pro['price'],0,',','.')?>&nbsp;<?=$currencyUnit?></strong>
				  </span> </td>
				</tr>
				<? if($pro['subject']!=0)
					{?>
...
$param="";
if ($p>1) echo '<a class="aLink3" title="'.$titleFirst.'" href="./?frame='.$_REQUEST['frame'].'&cat1='.$_REQUEST['cat1'].'&p=0">[&lt;&lt;]</a> ';
if ($p>0) echo '<a class="aLink3" title="'.$titlePrevious.'" href="./?frame='.$_REQUEST['frame'].'&cat1='.$_REQUEST['cat1'].'&p='.($p-1).'">[&lt;]</a> ';
$from=($p-10>0?$p-10:0);
$to=($p+10<$pages?$p+10:$pages);
for ($i=$from;$i<$to;$i++){
	if ($i!=$p) echo '<a class="aLink3" href="./?frame='.$_REQUEST['frame'].'&cat1='.$_REQUEST['cat1'].'&p='.$i.'">'.($i+1).' </a>';
	else echo '<span class="current"> '.($i+1).'</span> ';
}
if ($p<$i-1) echo '<a class="aLink3" title="'.$titleNext.'" href="./?frame='.$_REQUEST['frame'].'&cat1='.$_REQUEST['cat1'].'&p='.($p+1).'">[&gt;]</a> ';
if ($p<$pages-1) echo '<a class="aLink3" title="'.$titleLast.'" href="./?frame='.$_REQUEST['frame'].'&cat1='.$_REQUEST['cat1'].'&p='.($pages-1).'">[&gt;&gt;]</a>'; 
echo '</td></tr>';
?>
</table><br />
<? }?>
